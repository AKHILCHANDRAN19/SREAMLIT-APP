...
def pre_render_background(theme="blue"):
    ...
    r1, g1, b1, r2, g2, b2 = themes.get(theme, themes["blue"])
...
def pre_render_ribbon(title_text, cy_pos=280):
    layer = Image.new("RGBA", (WIDTH, HEIGHT), (0,0,0,0))
    draw = ImageDraw.Draw(layer)
    cx, cy = WIDTH//2, cy_pos
    w, h = 1040, 130 if cy_pos == 310 else 120
...
def render_bang_video(theme, prize_heading, item, lottery_title, out_path, duration_sec):
    ...
    ribbon_asset = pre_render_ribbon(prize_heading, cy_pos=310)
...
def render_scroll_video(theme, prize_heading, numbers_list, lottery_title, out_path, duration_sec, is_4col):
    print(f"**[LOG]** Multiprocessing 3-Core Render (Scroll): {out_path}", flush=True)
    total_frames = FPS * duration_sec
    cols = 4 if is_4col else 2
    
    bg_asset = pre_render_background(theme)
    ribbon_asset = pre_render_ribbon(prize_heading, cy_pos=280)
...
async def execute_result_pipeline(app, chat_id, target_url):
    ...
    # 3. Sequentially Render & Upload Individual Videos
    tier_config = [
    ...
    for p_name, engine, dur, is_4c, theme in tier_config:
        if p_name in prizes and prizes[p_name]:
            ...
            if engine == "bang":
                ...
                await asyncio.to_thread(render_scroll_video, theme, full_heading, prizes[p_name], lottery_title, out_path, dur, is_4c)
